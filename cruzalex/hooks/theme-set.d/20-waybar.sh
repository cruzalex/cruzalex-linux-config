...
generate_css_vars() {
    ...
    echo "@define-color background $(grep '^background' "$colors_file" | cut -d'"' -f2);" >> "$output_file"
    echo "@define-color foreground $(grep '^foreground' "$colors_file" | cut -d'"' -f2);" >> "$output_file"
    echo "@define-color accent $(grep '^accent' "$colors_file" | cut -d'"' -f2);" >> "$output_file"

    # Add ANSI colors
    for i in {0..15}; do
        local color=$(grep "^color$i" "$colors_file" | cut -d'"' -f2)
        if [ -n "$color" ]; then
